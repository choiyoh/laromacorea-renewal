    ...
    <v-card>
      ...
      <v-card-text>
        <v-form ref="form" v-model="valid" @submit.prevent="handleSubmit">
          ...
          <!-- Match 게시판 템플릿 -->
          <div v-if="formData.boardType === 'match' && !isEdit" class="match-template mb-4">
            <v-card variant="outlined">
              ...
              <v-card-text>
                <v-btn color="primary" variant="outlined" block @click="showMatchTemplate = true">
          ...
          <!-- 임시저장 알림 -->
          <v-alert v-if="draftSaved" type="info" variant="tonal" class="mb-4" dismissible>
            임시저장되었습니다.
          </v-alert>
        </v-form>
      </v-card-text>
      ...
      <v-card-actions class="pa-4">
        <v-btn variant="outlined" @click="saveDraft" :disabled="loading"> 임시저장 </v-btn>
        <v-spacer />
        <v-btn variant="outlined" @click="$emit('cancel')" :disabled="loading"> 취소 </v-btn>
        <v-btn
          color="primary"
          @click="handleSubmit"
          :loading="loading"
...
const boardOptions = [
  ...
  { title: '스쿼드', value: 'squad' },
  { title: '경기', value: 'match' },
  { title: '칼치오', value: 'calcio' },
  { title: '자유게시판', value: 'free' },
  { title: '스페셜', value: 'special' },
...
function onMediaUploadError({ file, error }) {
  ...
  alert(`${file.name} 업로드에 실패했습니다: ${error.message || '알 수 없는 오류'}`)
}
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.isEdit) {
      await postService.updatePost(props.post.id, postData)
    } else {
...
function saveDraft() {
  ...
  localStorage.setItem(`post_draft_${props.boardType || 'general'}`, JSON.stringify(draftData))
...
watch(
  ...
  () => {
    ...
    autoSaveInterval = setInterval(() => {
      if (formData.value.title || (quillEditor.value && quillEditor.value.getText().trim())) {
        saveDraft()
      }
    }, 30000)
  },
