<template>
  <v-card class="match-post-template" variant="outlined">
    ...
    <v-card-actions>
      ...
      <v-btn color="primary" :disabled="!canCreatePost" @click="handleCreatePost">
        게시글 작성
      </v-btn>
    </v-card-actions>
  </v-card>
...
const canCreatePost = computed(() => {
  return selectedMatch.value && postTitle.value.trim() && postContent.value.trim()
})
...
const previewContent = computed(() => {
  ...
  if (templateOptions.value.includeLineup) {
    content += '<br><br><strong>🔥 예상 라인업</strong><br>곧 업데이트 예정입니다!'
  }

  if (templateOptions.value.includeStats) {
    content += '<br><br><strong>📊 팀 통계</strong><br>최근 5경기 성적 및 주요 통계를 확인해보세요!'
  }

  if (templateOptions.value.includePrediction) {
    content += '<br><br><strong>🎯 경기 예측</strong><br>여러분의 예측을 댓글로 남겨주세요!'
  }

  if (templateOptions.value.enableCheering) {
    content += '<br><br><strong>📣 응원 메시지</strong><br>팀을 응원하는 메시지를 남겨주세요! 💪'
  }
...
function handleMatchSelect() {
  const match = props.availableMatches.find((m) => m.id === selectedMatch.value)
  if (match) {
...
function generateDefaultTitle(match) {
  const status =
    match.status === 'live' ? '[LIVE]' : match.status === 'finished' ? '[경기 결과]' : '[경기 예고]'

  postTitle.value = `${status} ${match.homeTeam.name} vs ${match.awayTeam.name} - 함께 응원해요! 🔥`
}

function generateDefaultContent(match) {
  const isRomaHome = match.homeTeam.name.includes('로마') || match.homeTeam.name.includes('Roma')
  const isRomaAway = match.awayTeam.name.includes('로마') || match.awayTeam.name.includes('Roma')
