    ...
    <v-card variant="outlined" class="upload-card">
      ...
      >
        ...
        <p
          ...
          :class="{ 'text-grey-lighten-2': disabled }"
        >
          {{ uploadText }}
        </p>
...
const acceptText = computed(() => {
  const imageTypes = props.acceptedTypes.filter((type) =>
    type.startsWith('image/'),
  );
  const videoTypes = props.acceptedTypes.filter((type) =>
